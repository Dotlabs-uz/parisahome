...
interface Props {
    ...
    id: number;
    setHandleUpdate: Dispatch<SetStateAction<boolean>>;
    good: any;
    categories: any;
}
...
export const PatchGoodModal: React.FC<Props> = ({
    children,
    id,
    setHandleUpdate,
    good,
    ...
    const onSubmit = async (formData: GoodForm) => {
        try {
            ...
            if (res.status === 200 || res.status === 201) {
                ...
                setHandleUpdate((prev) => (prev = !prev));
            }
        } catch (err: any) {
            console.error(err);
    ...
    return (
        <Dialog
            open={modalHandle}
            onOpenChange={(event) => setModalHandle(event)}
        >

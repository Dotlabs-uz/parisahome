...
const Accordion = ({ title, id, content, expanded, setExpanded }: any) => {
    ...
    return (
        <div className="w-full h-auto border-b cursor-pointer" ref={ref} onClick={() => setExpanded(id === expanded ? -1 : id)}>
            ...
            <div className="max-w-2xl overflow-hidden mb-3 max-sm:mb-2">
                <p className="text-[#1E1E1E99] max-sm:text-sm leading-7 max-sm:leading-5" dangerouslySetInnerHTML={{ __html: content }}></p>
            </div>
        </div>
    )
}
